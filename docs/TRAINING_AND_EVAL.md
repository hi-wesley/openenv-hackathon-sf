...
3. Decide whether the customer should be generated by the local Qwen model or by the offline mock backend.

Offline mock customer:

```bash
export DFA_AGENT_SIMULATOR_BACKEND=mock
```

Local Qwen customer:

Use the default `.env` and make sure `Qwen/Qwen2.5-0.5B-Instruct` is available locally.
...
5. In another shell, run the smoke test.
...
- Visit `http://localhost:7860/web` for the default OpenEnv interface
- Visit `http://localhost:7860/dashboard` for the customer-service dashboard

7. Test one episode.

- Pick one of the three customer-service scenarios
- Use `default_policy`
- Step until the episode ends
- Confirm the customer emotion scores update each turn
...
- Complete an episode in `/dashboard`
...
5. Confirm `/web` renders.
6. Confirm `/dashboard` renders.
7. Confirm the simulator backend works if configured.
...
4. Generate before/after examples for the `/dashboard` comparator.

## Notes

- The built-in default scorer is `emotion_balance`, which uses the final customer message as a lightweight proxy.
- For training, shaped rewards are available immediately.
- The default local setup uses `Qwen/Qwen2.5-0.5B-Instruct` for customer simulation and the training assistant model.
